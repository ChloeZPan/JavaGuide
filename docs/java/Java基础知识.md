...
1. 简单易学；
2. 面向对象（封装，继承，多态）；
    encapsulation(data hidding, wrapping variables and methods as a single unit, accesssed only thorough its current methods);
    inheritance(superclass and subclass);
    polymorphism(overloading: diff signature and overriding);
    abstraction(abstract classes and interface: static final + all methods are abstract);
3. 平台无关性（ Java 虚拟机实现平台无关性）；bytecode
4. 可靠性；
5. 安全性；
    严格遵循面向对象的规范、无指针运算、数组边界检查、强制类型转换检查等；
...
8. 编译与解释并存(compiling and interpreted);
...
如果你只是为了运行一下 Java 程序的话，那么你只需要安装 JRE 就可以了。如果你需要进行一些 Java 编程方面的工作，那么你就需要安装JDK了。但是，这不是绝对的。有时，即使您不打算在计算机上进行任何Java开发，仍然需要安装JDK。例如，如果要使用JSP部署Web应用程序，那么从技术上讲，您只是在应用程序服务器中运行Java程序。那你为什么需要JDK呢？因为应用程序服务器会将 JSP 转换为 Java servlet，并且需要使用 JDK 来编译 servlet。JSP在第一次被访问的时候会被编译为HttpJspPage类（该类是HttpServlet的一个子类）；
...
- Java 的类是单继承的 single inheritance，C++ 支持多重继承；虽然 Java 的类不可以多继承，但是接口可以多继承。
...
> Java中基本数据类型所占内存大小
![](https://blog.csdn.net/qq_36894974/article/details/79254715)
